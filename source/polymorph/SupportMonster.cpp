...
int SupportMonster::decision(Battlefield* &battlefield) {
  ...
  closest_enemy_points *= SUPPORT_ATTACK_COST;
  weakest_enemy_points *= SUPPORT_ATTACK_COST;
  closest_cover_points *= SUPPORT_COVER_COST;
  biggest_cover_points *= SUPPORT_COVER_COST;
...
int SupportMonster::evaluate(Battlefield* &battlefield, int &to_x, int &to_y, int &points) {
  points = 0;
  if (battlefield != NULL) {
      ...
      if (battlefield->_map != NULL) {
        ...
        if (battlefield->_map->_map[to_x][to_y]->_on_tile == NULL) {
		  ...
		  if (is_merc != FREE_INDEX) {
            ...
            //some code for interaction with allies here (support monster should try to heal weakest ally in his reach)
		  }
		}
	  }
	}
