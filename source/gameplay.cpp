...
void gameplay::copy_location_data(struct TLocationIgData *from, struct TLocationIgData *to)
{
...
void gameplay::swap_location_data(TLocationIgData *from,  TLocationIgData *to)
{
  TLocationIgData buf;
...
void gameplay::clear_location_data(TLocationIgData *data)
{
  for (int j = 0; j < PAIR_ARR_SIZE; ++j)
  {
  	for (int i = 0; i < MAX_PACKS_COUNT; ++i)
...
gameplay::TLocationIgData gameplay::Location::generate(database::TLocationStData *tmp, player::Player *pl)
{
...
bool gameplay::Location::on_enter(TLocationIgData *curr_loc)
{
...
gameplay::GlobalMap::GlobalMap(TLocationIgData *inp_map, int map_sz, short plr_pos)
{
...
short gameplay::GlobalMap::get_player_position()
{
  return player_position;
}

void gameplay::GlobalMap::set_player_position(short value)
{
  player_position = value;
}

gameplay::TLocationIgData gameplay::GlobalMap::get_location(int ind)
{
  return map[ind];
}

void gameplay::GlobalMap::set_location(TLocationIgData *inp_loc, int inp_sz, int start_ind)
{
